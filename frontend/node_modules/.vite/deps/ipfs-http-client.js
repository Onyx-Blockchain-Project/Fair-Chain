...
function is(value) {
  ...
  if (typeOf2 === "string" || typeOf2 === "number" || typeOf2 === "bigint" || typeOf2 === "symbol") {
    return typeOf2;
  }
  if (typeOf2 === "function") {
    return "Function";
...
var fromString = useBuffer ? (
   ...
   */
  (string3) => {
    return string3.length >= FROM_STRING_THRESHOLD_BUFFER ? (
      // eslint-disable-line operator-linebreak
      // @ts-ignore
   ...
   */
  (string3) => {
    return string3.length >= FROM_STRING_THRESHOLD_TEXTENCODER ? textEncoder3.encode(string3) : utf8ToBytes(string3);
  }
);
var fromArray = (arr) => {
...
function toToken(data, pos, prefix, length7) {
  ...
  const buf = data.slice(pos + prefix, pos + prefix + length7);
  return new Token(Type.bytes, buf, prefix + length7);
}
...
function toStr(bytes, start, end) {
  ...
  if (len < ASCII_THRESHOLD) {
    ...
    for (let i = start; i < end; i++) {
      const c = bytes[i];
      if (c & 128) {
        return textDecoder2.decode(bytes.subarray(start, end));
      }
...
var decode17 = (data) => {
  const buf = toByteView3(data);
  const options = Object.assign(decodeOptions2, { tokenizer: new DagJsonTokenizer(buf, decodeOptions2) });
  return decode12(buf, options);
};
...
var toString4 = function(buf, offset = 0, length7) {
  offset = ~~offset;
  length7 = length7 ?? buf.length - offset;
...
function str2bytes(str) {
  ...
  const size = Uint8Array.from(import_varint6.default.encode(buf.length));
  return concat2([size, buf], size.length + buf.length);
}
function bytes2str(buf) {
  const size = import_varint6.default.decode(buf);
  buf = buf.slice(import_varint6.default.decode.bytes);
  if (buf.length !== size) {
...
function bytes2mb(buf) {
  const size = import_varint6.default.decode(buf);
  const hash = buf.slice(import_varint6.default.decode.bytes);
  if (hash.length !== size) {
    throw new Error("inconsistent lengths");
  }
...
function bytes2mh(buf) {
  const size = import_varint6.default.decode(buf);
  const address = buf.slice(import_varint6.default.decode.bytes);
...
function onion2bytes(str) {
  ...
  const buf = base324.decode("b" + addr[0]);
...
function tuplesToBytes(tuples) {
  return fromBytes(concat2(tuples.map((tup) => {
    const proto = protoFromTuple(tup);
    let buf = Uint8Array.from(import_varint7.default.encode(proto.code));
    if (tup.length > 1 && tup[1] != null) {
...
function bytesToTuples(buf) {
  ...
  let i = 0;
  while (i < buf.length) {
    const code10 = import_varint7.default.decode(buf, i);
...
var DefaultMultiaddr = class _DefaultMultiaddr {
  ...
  protoCodes() {
    ...
    let i = 0;
    while (i < buf.length) {
      const code10 = import_varint8.default.decode(buf, i);
    ...
    try {
      ...
      if ((tuple == null ? void 0 : tuple[1]) != null) {
        ...
        if (peerIdStr[0] === "Q" || peerIdStr[0] === "1") {
          return toString3(base58btc4.decode(`z${peerIdStr}`), "base58btc");
        }
        return toString3(CID4.parse(peerIdStr).multihash.bytes, "base58btc");
      }
      return null;
    } catch (e) {
      return null;
...
function decode38(buf, offset = 0) {
  if (buf instanceof Uint8Array) {
...
function bytesToString2(base7) {
  return (buf) => {
    return toString6(buf, base7);
  };
}
function stringToBytes2(base7) {
  return (buf) => {
    return fromString7(buf, base7);
  };
}
function bytes2port2(buf) {
  const view = new DataView(buf.buffer);
...
function onion2bytes2(str) {
  ...
  const buf = fromString7(addr[0], "base32");
  const port = parseInt(addr[1], 10);
  if (port < 1 || port > 65536) {
    throw new Error("Port number is not in range(1, 65536)");
  }
...
var ip6ToBytes = function(ip) {
  ...
  for (i = 0; i < sections.length; i++) {
    ...
    if (isv4) {
      ...
      sections[i] = toString6(v4Buffer.subarray(0, 2), "base16");
    }
    if (v4Buffer != null && ++i < 8) {
      sections.splice(i, 0, toString6(v4Buffer.subarray(2, 4), "base16"));
    }
  }
  if (sections[0] === "") {
    while (sections.length < 8) {
...
function bytes2mb2(base7) {
  return (buf) => {
    return base7.encoder.encode(buf);
  };
}
...
function bytesToComponents(bytes) {
  ...
  while (i < bytes.length) {
    ...
    if (size > 0) {
      ...
      component.value = ((_a8 = codec.bytesToValue) == null ? void 0 : _a8.call(codec, valueBytes)) ?? toString6(valueBytes);
    }
...
function peerIdFromBytes(buf) {
  try {
    ...
    return peerIdFromCID(CID4.decode(buf));
  }
...
var rpcToBigInt = (mb) => BigInt(`0x${toString3(base64url2.decode(mb), "base16")}`);
